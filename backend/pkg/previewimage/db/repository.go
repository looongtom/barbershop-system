...
func (r repo) UploadImages(ctx context.Context, previewImage entity.PreviewImage) (*entity.PreviewImage, error) {
	...
	err := r.db.QueryRowContext(ctx, query, previewImage.AccountId, previewImage.SelfImg, previewImage.ShapeImg, previewImage.ColorImg, previewImage.CreatedAt).Scan(&resp.ID, &resp.AccountId, &resp.SelfImg, &resp.ShapeImg, &resp.ColorImg, &resp.CreatedAt)
	if err != nil {
		return nil, err
	}
	return &resp, nil
